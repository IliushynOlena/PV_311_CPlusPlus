...
protected://private
    ...
    int health;
    float strength;
    static const int maxHelth = 100;
public:
    Hero():name("no name"),health(maxHelth), strength(0){}
    Hero(string name, float st):name(name),health(maxHelth), strength(st){}
    void Print()const
    {
        cout << "Name : " << name << endl;
...
int main()
{
    ...
    Dragon boss("Oleg",rand()%6 +5, rand() % 5 + 1);
    boss.Print();

    //Dragon dragon("Vasia", rand() % 6 + 5, rand() % 5 + 1);
    //dragon.Print();
    //int i = 1;
    //do
    //{
